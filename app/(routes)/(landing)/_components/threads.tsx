...
interface ThreadsProps {
  /** Explicit RGB override (0–1 per channel). Takes precedence over the theme colors. */
  color?: [number, number, number];
  /** Thread color used in light mode (hex). */
  lightColor?: string;
  /** Thread color used in dark mode (hex). */
  darkColor?: string;
  amplitude?: number;
  distance?: number;
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  // Explicit `color` wins; otherwise derive the thread color from the active theme.
  const effectiveColor: [number, number, number] =
    color ?? hexToRgb(resolvedTheme === 'dark' ? darkColor : lightColor);

  // Keep the latest props in a ref so updating them (e.g. on theme switch) mutates
  // the live shader uniforms instead of tearing down and rebuilding the WebGL context.
  const propsRef = useRef({ color: effectiveColor, amplitude, distance, enableMouseInteraction });
  useEffect(() => {
    propsRef.current = { color: effectiveColor, amplitude, distance, enableMouseInteraction };
  });
